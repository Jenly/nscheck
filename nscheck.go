...
//	}
	...
	fmta := func(d time.Duration) string {
		//return color.CyanString("%7d", int(d/time.Millisecond))
		return strconv.Itoa(int(d/time.Millisecond))
	}

//	fmtb := func(d time.Duration) string {
//		return color.CyanString("%-9s", strconv.Itoa(int(d/time.Millisecond)))
//	}
	...
	switch url.Scheme {
	case "https":
		var dns = fmta(t1.Sub(t0))
		var tcp = fmta(t2.Sub(t1))
		var tls = fmta(t6.Sub(t5))
		var ser = fmta(t4.Sub(t3))
		var tra = fmta(t7.Sub(t4))
		var total = fmta(t7.Sub(t0))
        fmt.Println(total,dns,tcp,ser,tra,tls)
        //fmt.Println(dns)
        //fmt.Println(tcp)
        //fmt.Println(tls)
        //fmt.Println(ser)
        //fmt.Println(tra)
	case "http":
		var dns = fmta(t1.Sub(t0)) // dns lookup
		var tcp = fmta(t3.Sub(t1)) // tcp connection
		var ser = fmta(t4.Sub(t3)) // server processing
		var tra = fmta(t7.Sub(t4)) // content transfer
		var total = fmta(t7.Sub(t0)) // total
        fmt.Println(total,dns,tcp,ser,tra)
	}
	//fmt.Println()
